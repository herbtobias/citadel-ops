...
const EVENT_TO_NOTIFICATION: Record<string, (typeof import('../db/schema').notificationType.enumValues)[number]> = {
...
export default defineNitroPlugin(() => {
  subscribeEvents((evt) => {
    ...
    if (notifType && evt.projectId) {
      notifyProject(evt.projectId, notifType, { missionId: evt.missionId, message: evt.message }).catch(() => {})
    }
