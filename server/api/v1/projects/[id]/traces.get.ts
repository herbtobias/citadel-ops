...
export default defineEventHandler(async (event) => {
  ...
  const actorOf = (r: (typeof rows)[number]) =>
    r.actorType === 'agent'
  ...
  const errs = traceIds.length
    ? await db
        ...
        .where(inArray(schema.errorEvents.traceId, traceIds))
    : []

  return traceIds.map((t) => {
    const entries = byTrace.get(t)!.slice().reverse() // chronological within the trace
    const first = entries[0]
    if (!first) throw createError({ statusCode: 500, statusMessage: 'Empty trace group' })
    return {
      traceId: t,
      ...
      events: entries.map((e) => ({
        event: e.event,
        missionKey: e.missionId ? (mKey.get(e.missionId) ?? null) : null,
