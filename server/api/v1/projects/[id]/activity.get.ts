...
export default defineEventHandler(async (event) => {
  ...
  const limit = Math.min(Number.parseInt((getQuery(event).limit as string) || '100', 10) || 100, 500)
  ...
  const licenses = licenseIds.length ? await db.select({ id: schema.licenses.id, alias: schema.licenses.agentAlias }).from(schema.licenses).where(inArray(schema.licenses.id, licenseIds)) : []
  ...
  return rows.map(r => ({
    ...
    toStatus: r.toStatus,
    message: r.message,
    actor: r.actorType === 'agent'
      ? (r.actorLicenseId ? `agent ${lAlias.get(r.actorLicenseId) ?? '?'}` : 'agent')
      : r.actorType === 'human'
        ? (r.actorUserId ? uName.get(r.actorUserId) ?? 'HQ' : 'HQ')
        : 'system',
