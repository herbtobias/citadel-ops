...
export default defineEventHandler(async (event) => {
  ...
  const ops = opIds.length ? await db.select({ id: schema.operations.id, key: schema.operations.key }).from(schema.operations).where(inArray(schema.operations.id, opIds)) : []
  ...
  for (const d of deps) {
    ...
    byAgent[a] = { tokens: (byAgent[a]?.tokens ?? 0) + (d.tokensSpent ?? 0), cost: (byAgent[a]?.cost ?? 0) + (d.costSpent ?? 0) }
    byOperation[o] = { tokens: (byOperation[o]?.tokens ?? 0) + (d.tokensSpent ?? 0), cost: (byOperation[o]?.cost ?? 0) + (d.costSpent ?? 0) }
  }
