...
export default defineEventHandler(async (event) => {
  ...
  const wire = await db
    .select({ fromStatus: schema.activityLog.fromStatus, toStatus: schema.activityLog.toStatus })
    .from(schema.activityLog)
    .where(eq(schema.activityLog.projectId, projectId))
  const rework = wire.filter(
    (w) =>
      (w.fromStatus === 'in_review' && w.toStatus === 'in_progress') ||
      (w.fromStatus === 'cold_read' && w.toStatus === 'designing'),
  ).length
  ...
  const deps = await db
    ...
    .where(eq(schema.missions.projectId, projectId))
  const spend = deps.reduce(
    (a, d) => ({ tokens: a.tokens + (d.tokensSpent ?? 0), cost: a.cost + (d.costSpent ?? 0) }),
    { tokens: 0, cost: 0 },
  )
  ...
  return {
    ...
    agents: {
      ...
      revoked: licenses.length - activeLicenses.length,
    },
  }
})
