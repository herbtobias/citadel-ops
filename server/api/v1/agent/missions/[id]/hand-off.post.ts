...
export default defineEventHandler(async (event) => {
  ...
  if (source.claimedByLicenseId !== lic.id) {
    throw createError({
      statusCode: 403,
      statusMessage: 'Hand-off requires owning the source mission',
    })
  }
  ...
  if (newDepth > maxDepth) {
    throw createError({
      statusCode: 422,
      statusMessage: `Hand-off depth ${newDepth} exceeds max ${maxDepth}`,
    })
  }

  // Pull source artifacts so they can be shared with the new mission.
  const srcArtifacts = await db
    .select()
  ...
  const run = async () => {
    ...
    const existing = await db
      ...
      .where(eq(schema.missions.projectId, source.projectId))
    ...
    await logActivity({
      ...
      event: 'handed_off',
      message: `Handed off ${key} (${body.sector}/${body.type}) — ${body.linkType} ${source.key}`,
    ...
    await logActivity({
      ...
      event: 'created',
      toStatus: 'ready',
      message: `Spawned from ${source.key} via hand-off`,
    })
