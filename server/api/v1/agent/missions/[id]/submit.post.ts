...
export default defineEventHandler(async (event) => {
  ...
  await db.update(schema.missions).set({ status: 'in_review', updatedAt: new Date() }).where(eq(schema.missions.id, id))
  await logActivity({
    projectId: m.projectId, missionId: id, actorType: 'agent', actorLicenseId: lic.id,
    event: 'submitted_for_review', fromStatus: m.status, toStatus: 'in_review', message: `${lic.agentAlias} submitted for review`,
  })
