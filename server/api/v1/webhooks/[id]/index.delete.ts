...
export default defineEventHandler(async (event) => {
  ...
  const [sub] = await db.select().from(schema.webhookSubscriptions).where(eq(schema.webhookSubscriptions.id, id))
  if (!sub) throw createError({ statusCode: 404, statusMessage: 'Webhook not found' })
  const [project] = await db.select().from(schema.projects).where(eq(schema.projects.id, sub.projectId))
