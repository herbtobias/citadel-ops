...
export default defineEventHandler(async (event) => {
  ...
  if (actor.kind === 'agent' && mission.claimedByLicenseId !== actor.license.id) {
    throw createError({ statusCode: 403, statusMessage: 'Filing a dossier requires owning the mission claim' })
  }
  const body = await parseBody(event, schema_)

  const [dossier] = await db.insert(schema.dossiers).values({
  ...
  await logActivity({
    projectId: mission.projectId, missionId, actorType: actor.kind === 'agent' ? 'agent' : 'human',
    actorLicenseId: actor.license?.id, actorUserId: actor.user?.id,
    event: 'dossier_filed', fromStatus: mission.status, toStatus: patch.status as string | undefined,
