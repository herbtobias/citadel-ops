...
const schema_ = z.object({
  key: z.string().min(1).max(8).regex(/^[A-Z][A-Z0-9]*$/, 'uppercase letters/digits, e.g. WEB'),
...
export default defineEventHandler(async (event) => {
  ...
  const [dup] = await db.select().from(schema.projects)
    .where(and(eq(schema.projects.orgId, orgId), eq(schema.projects.key, body.key)))
  if (dup) throw createError({ statusCode: 409, statusMessage: `Project key ${body.key} already exists in this org` })
  ...
  const [project] = await db.insert(schema.projects).values({
    orgId, key: body.key, name: body.name, description: body.description, settings,
  }).returning()
