...
export async function listAccessibleProjects(user: SessionUser) {
  ...
  const all = memberOrgIds.length
    ? await db.select().from(projects).where(inArray(projects.orgId, memberOrgIds))
    : []
...
export async function assertProjectWrite(
  ...
  if (ctx.orgRole === 'viewer')
    throw createError({ statusCode: 403, statusMessage: 'Viewers are read-only' })
  return ctx
}
...
export async function assertOrgManager(event: H3Event, orgId: string): Promise<SessionUser> {
  ...
  if (role !== 'manager')
    throw createError({ statusCode: 403, statusMessage: 'Manager role required' })
  return user
}
