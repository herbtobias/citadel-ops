...
export async function dispatchWebhooks(evt: CitadelEvent) {
  ...
  const subs = await db
    ...
    .where(
      and(eq(webhookSubscriptions.projectId, evt.projectId), eq(webhookSubscriptions.active, true)),
    )

  for (const sub of subs) {
    const subscribed =
      sub.events.length === 0 || sub.events.includes('*') || sub.events.includes(evt.type)
    if (!subscribed) continue

    const body = JSON.stringify(evt)
    const headers: Record<string, string> = { 'content-type': 'application/json' }
    if (sub.secret)
      headers['x-citadel-signature'] =
