...
export async function requireLicense(event: H3Event): Promise<License> {
  ...
  if (!token) throw createError({ statusCode: 401, statusMessage: 'Missing license (Bearer token)' })

  const [lic] = await db.select().from(licenses).where(eq(licenses.hashedKey, hashLicenseKey(token)))
  if (!lic) throw createError({ statusCode: 401, statusMessage: 'Invalid license' })

  if (lic.status === 'revoked') throw createError({ statusCode: 401, statusMessage: 'license_revoked' })
  if (lic.status === 'expired') throw createError({ statusCode: 401, statusMessage: 'license_expired' })
...
export async function sweepExpiredLeases(projectId: string): Promise<number> {
  const expired = await db.select().from(missions).where(and(
  ...
  for (const m of expired) {
    ...
    await logActivity({
      projectId, missionId: m.id, actorType: 'system',
      event: 'lease_expired', fromStatus: 'in_progress', toStatus: 'ready',
...
export async function withIdempotency<T>(
  ...
  const [existing] = await db.select().from(schema.idempotencyKeys)
