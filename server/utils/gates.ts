...
export async function checkGates(projectId: string, mission: Mission, toStatus: string): Promise<void> {
  const gates = await db.select().from(qualityGates).where(and(
  ...
  for (const g of gates) {
    ...
    if (rule.requireGoldfish) {
      if (!mission.dossierId) {
        throw createError({ statusCode: 422, statusMessage: `Gate "${g.name}": a Cold-Read-passed dossier is required` })
      }
      const [d] = await db.select().from(dossiers).where(eq(dossiers.id, mission.dossierId))
      if (d?.status !== 'cold_read_passed') {
        throw createError({ statusCode: 422, statusMessage: `Gate "${g.name}": dossier has not passed Cold Read` })
      }
    }

    if (rule.requireArtifacts) {
      const arts = await db.select({ id: artifacts.id }).from(artifacts).where(eq(artifacts.missionId, mission.id))
      if (arts.length === 0) {
        throw createError({ statusCode: 422, statusMessage: `Gate "${g.name}": at least one artifact is required` })
      }
    }

    if (rule.requireHarnessPass) {
      const reports = await db.select({ id: artifacts.id }).from(artifacts)
        .where(and(eq(artifacts.missionId, mission.id), eq(artifacts.kind, 'test_report')))
      if (reports.length === 0) {
        throw createError({ statusCode: 422, statusMessage: `Gate "${g.name}": a passing harness report (test_report artifact) is required` })
      }
    }
    ...
    if (rule.requireAcceptanceChecked) {
      ...
      if (!mission.acceptanceCriteria || mission.acceptanceCriteria.length === 0) {
        throw createError({ statusCode: 422, statusMessage: `Gate "${g.name}": acceptance criteria must be defined` })
      }
    }
  }
}
