...
export async function resolveProjectActor(event: H3Event, projectId: string): Promise<ProjectActor> {
  if (getBearerToken(event)) {
    const license = await requireLicense(event)
    if (license.projectId && license.projectId !== projectId) {
      throw createError({ statusCode: 403, statusMessage: 'License is scoped to a different project' })
    }
