...
async function accept() {
  if (!token.value) { error.value = 'Missing invite token'; return }
  error.value = ''
  loading.value = true
  try {
    await $fetch('/api/v1/invitations/accept', {
      method: 'POST',
      body: { token: token.value, name: name.value || undefined, password: password.value || undefined },
    })
    ...
    <div class="ct-card w-full max-w-sm border border-border bg-card p-8">
      ...
      <template v-else>
        <p class="ct-label mb-6 text-muted-foreground">
          Set credentials to accept. Leave blank if you already have an account
        ...
        <form class="space-y-4" @submit.prevent="accept">
          <div>
            ...
            <input v-model="name" type="text" class="w-full rounded-[var(--radius-card)] border border-border bg-background px-3 py-2 text-sm focus:border-accent focus:outline-none">
          </div>
          <div>
            <label class="ct-label mb-1 block text-muted-foreground">Password</label>
            <input v-model="password" type="password" autocomplete="new-password" class="w-full rounded-[var(--radius-card)] border border-border bg-background px-3 py-2 text-sm focus:border-accent focus:outline-none">
          </div>
          <p v-if="error" class="ct-label text-destructive">{{ error }}</p>
          <button type="submit" :disabled="loading" class="ct-glow-sm w-full rounded-[var(--radius-card)] bg-accent px-4 py-2 text-sm font-bold text-background hover:opacity-90 disabled:opacity-50">
