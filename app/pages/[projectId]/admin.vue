...
interface Metrics { counts: { total: number, byStatus: Record<string, number> }, reviewQueue: number, blocked: number, done: number, agents: { active: number, total: number } }
interface Finops { total: { tokens: number, cost: number }, runs: number, byAgent: Record<string, { tokens: number, cost: number }> }
interface TraceEvent { event: string, missionKey: string | null, fromStatus: string | null, toStatus: string | null, message: string | null, createdAt: string }
interface Trace { traceId: string, startedAt: string, actor: string, actorType: string, eventCount: number, errorCount: number, events: TraceEvent[], errors: { source: string, message: string }[] }
...
const { data, refresh } = await useAsyncData(
  ...
  async () => {
    ...
    const rollup = await Promise.all(orgProjects.value.map(async p => ({
      key: p.key, name: p.name, metrics: await f<Metrics>(`/api/v1/projects/${p.id}/metrics`),
    })))
...
<template>
  <div class="mx-auto max-w-5xl space-y-8">
    ...
    <p v-if="!isManager" class="ct-card border border-border bg-card p-4 text-sm text-muted-foreground">
      Admin metrics &amp; tracing are manager-only. Your role: <span class="text-accent">{{ orgs.activeRole ?? '—' }}</span>.
    </p>
    ...
    <template v-else-if="data">
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="ct-label border-b border-border text-left text-muted-foreground">
              <th class="py-2">Project</th><th>Missions</th><th>Done</th><th>Review</th><th>Blocked</th><th>Agents</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="r in data.rollup" :key="r.key" class="border-b border-border/50">
              <td class="py-2"><span class="ct-label text-accent">{{ r.key }}</span> {{ r.name }}</td>
      ...
      <section class="ct-card border border-border bg-card p-5">
        ...
        <div class="mb-3 flex gap-6">
          <div><p class="ct-label text-muted-foreground">Token spend</p><p class="text-lg font-bold">{{ data.finops.total.tokens.toLocaleString('en-US') }}</p></div>
          <div><p class="ct-label text-muted-foreground">Cost</p><p class="text-lg font-bold">{{ data.finops.total.cost }}</p></div>
          <div><p class="ct-label text-muted-foreground">Agent runs</p><p class="text-lg font-bold">{{ data.finops.runs }}</p></div>
        </div>
        <div class="flex flex-wrap gap-2">
          <span v-for="(v, agent) in data.finops.byAgent" :key="agent" class="ct-label rounded bg-muted px-2 py-1 text-muted-foreground">
      ...
      <section class="ct-card border border-border bg-card p-5">
        <div class="mb-3 flex items-center justify-between">
          <h2 class="ct-label text-muted-foreground">Trace Log — agent &amp; HQ requests (by traceId)</h2>
          <span class="ct-label flex items-center gap-1.5 text-muted-foreground"><span class="size-2 rounded-full bg-accent ct-glow-sm" /> live</span>
        </div>
        <ul class="space-y-1">
          <li v-for="t in data.traces" :key="t.traceId" class="border-b border-border/50">
            <button class="flex w-full items-center justify-between py-2 text-left text-sm hover:text-accent" @click="toggle(t.traceId)">
              <span class="flex items-center gap-2">
                <Icon :name="open.has(t.traceId) ? 'lucide:chevron-down' : 'lucide:chevron-right'" class="size-4" />
                <span class="ct-label" :class="actorColor[t.actorType]">{{ t.actor }}</span>
                <span class="text-muted-foreground">{{ t.events.map(e => e.event).slice(0, 3).join(', ') }}{{ t.eventCount > 3 ? '…' : '' }}</span>
              </span>
              <span class="flex items-center gap-2">
                <span v-if="t.errorCount" class="ct-label text-destructive">{{ t.errorCount }} err</span>
            ...
            <div v-if="open.has(t.traceId)" class="ml-6 space-y-1 pb-3 text-xs">
              <div v-for="(e, i) in t.events" :key="i" class="flex gap-2">
                ...
                <span v-if="e.fromStatus || e.toStatus" class="text-muted-foreground">{{ e.fromStatus }}→{{ e.toStatus }}</span>
                <span v-if="e.message" class="text-muted-foreground">— {{ e.message }}</span>
              </div>
              <div v-for="(er, i) in t.errors" :key="'e' + i" class="text-destructive">✗ {{ er.source }}: {{ er.message }}</div>
            </div>
          </li>
          <li v-if="!data.traces.length" class="py-3 text-sm text-muted-foreground">No traced requests yet.</li>
        </ul>
      </section>
    </template>
  </div>
