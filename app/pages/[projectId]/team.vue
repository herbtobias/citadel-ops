...
const { data, refresh, pending } = await useAsyncData(
  ...
  async () => {
    ...
    return useRequestFetch()<{ members: Member[]; invitations: Invite[] }>(
      `/api/v1/organizations/${orgs.activeOrgId}/members`,
    )
  },
    ...
    <template v-else>
      ...
      <section class="ct-card border border-border bg-card p-5">
        ...
        <p v-if="lastInvite" class="ct-label mt-3 text-muted-foreground">
          Invite created for <span class="text-accent">{{ lastInvite.email }}</span
          >. Share this link (email delivery lands in P8):
        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="m in data?.members" :key="m.userId" class="border-b border-border/50">
              ...
              <td>
                <span class="text-accent">{{ m.role }}</span>
              </td>
      ...
      <section v-if="data?.invitations?.length" class="ct-card border border-border bg-card p-5">
        ...
        <ul class="space-y-2 text-sm">
          <li
            v-for="i in data.invitations"
            :key="i.id"
            class="flex items-center justify-between border-b border-border/50 pb-2"
          >
            <span
              >{{ i.email }} · <span class="text-accent">{{ i.role }}</span></span
            >
            <code class="ct-label text-xs text-muted-foreground"
              >/accept-invite?token={{ i.token }}</code
            >
          </li>
        </ul>
      </section>
