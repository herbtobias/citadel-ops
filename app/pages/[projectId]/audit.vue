  ...
  try {
    await $fetch(`/api/v1/projects/${projectId.value}/orders`, {
      ...
      body: {
        ...
        broadcast: true,
        payload: orderMsg.value ? { message: orderMsg.value } : undefined,
      },
    })
    orderMsg.value = ''
    await refresh()
    ...
    <section class="ct-card border border-border bg-card p-5">
      ...
      <ul class="space-y-2">
        <li
          v-for="e in entries"
          :key="e.id"
          class="flex items-start gap-3 border-b border-border/50 pb-2 text-sm"
        >
          ...
          <div class="min-w-0">
            ...
            <span v-if="e.fromStatus || e.toStatus" class="ct-label ml-1 text-muted-foreground"
