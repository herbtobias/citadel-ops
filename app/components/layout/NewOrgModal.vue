...
watch(name, (n) => {
  if (!slugTouched) slug.value = n.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')
})
...
async function submit() {
  ...
  try {
    ...
    emit('created', org.id)
    name.value = ''; slug.value = ''; ownerEmail.value = ''; slugTouched = false
  }
  catch (e: any) {
        ...
        <div class="ct-card w-full max-w-md border border-border bg-card p-6">
          ...
          <form class="space-y-3" @submit.prevent="submit">
            <div>
              ...
              <input v-model="name" required placeholder="Night Division"
            ...
            <div>
              ...
              <input v-model="slug" required pattern="[a-z0-9-]+" placeholder="night-division" @input="slugTouched = true"
            ...
            <div>
              <label class="ct-label mb-1 block text-muted-foreground">Owner email (existing user → becomes Manager)</label>
              <input v-model="ownerEmail" type="email" required placeholder="owner@example.com"
                class="w-full rounded-[var(--radius-card)] border border-border bg-background px-3 py-2 text-sm focus:border-accent focus:outline-none">
            </div>
